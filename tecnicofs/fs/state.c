...
pthread_mutex_t trinco_free_inode; 
...
pthread_mutex_t trinco_free_ofe;
...
void state_init() {
    for (int i = 0; i<INODE_TABLE_SIZE;i++){
        pthread_rwlock_init(&inode_table[i].trinco, NULL);
    }
    for (int i = 0; i<MAX_OPEN_FILES;i++){
        pthread_rwlock_init(&open_file_table[i].trinco, NULL);
    }
    pthread_mutex_init(&trinco_free_ofe,NULL);
    pthread_mutex_init(&trinco_free_inode, NULL);
...
void state_destroy() {
    for (int i = 0; i<INODE_TABLE_SIZE;i++){
        pthread_rwlock_destroy(&inode_table[i].trinco);
    }
    for (int i = 0; i<MAX_OPEN_FILES;i++){
        pthread_rwlock_destroy(&open_file_table[i].trinco);
    }
    pthread_mutex_destroy(&trinco_free_ofe);
    pthread_mutex_destroy(&trinco_free_inode);
}
        ...
        if (freeinode_ts[inumber] == FREE) {
            ...
            if (n_type == T_DIRECTORY) {
                ...
                if (b == -1) {
                    ...
                    pthread_rwlock_unlock(&inode_table[inumber].trinco);
                    pthread_mutex_unlock(&trinco_free_inode);
                    return -1;
                }
    ...
    if (inode_table[inumber].i_size > 0) {
        for(int i = 0; i<266;i++){
            if (i<10){
                ...
                int* indirect_block = data_block_get(inode_table[inumber].i_x_data_block);
                int index = i - 10;
...
int add_to_open_file_table(int inumber, size_t offset) {
    pthread_mutex_lock(&trinco_free_ofe);
    for (int i = 0; i < MAX_OPEN_FILES; i++) {
        pthread_rwlock_wrlock(&open_file_table[i].trinco);
        if (free_open_file_entries[i] == FREE) {
            free_open_file_entries[i] = TAKEN;
            open_file_table[i].of_inumber = inumber;
            open_file_table[i].of_offset = offset;
            pthread_mutex_unlock(&trinco_free_ofe);
            pthread_rwlock_unlock(&open_file_table[i].trinco);
            return i;
        }
        pthread_rwlock_unlock(&open_file_table[i].trinco);
    }
